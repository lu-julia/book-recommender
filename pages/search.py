...
def show():
    ...
    search_query = st.text_input(
        "Search by title or author", placeholder="Enter title or author name"
    )
    ...
    with col1:
        year_from = st.number_input(
            "Year from", min_value=year_min, max_value=year_max, value=year_min
        )
    with col2:
        year_to = st.number_input(
            "Year to", min_value=year_min, max_value=year_max, value=year_max
        )
