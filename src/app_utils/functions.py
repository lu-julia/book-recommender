...
def add_book(df_user: pd.DataFrame, user_id: int, isbn: str) -> pd.DataFrame:
    ...
    """
    new_row = {"user_id": user_id, "ISBN": isbn, "rating": 0.0}
...
def add_rating(df_user: pd.DataFrame, row: pd.Series, rating: float) -> pd.DataFrame:
    ...
    """
    df_user.loc[
        (df_user["ISBN"] == row["ISBN"]) & (df_user["user_id"] == row["user_id"]),
        "rating",
    ] = rating
    return df_user
...
def update_books(
    df_user: pd.DataFrame, df_ratings: pd.DataFrame, user_id: int
) -> pd.DataFrame:
    """
    ...
    for _, row in df_user.iterrows():
        if row["ISBN"] not in result["ISBN"].tolist():
            new_row = {"user_id": user_id, "ISBN": row["ISBN"], "rating": row["rating"]}
            df_new_row = pd.DataFrame([new_row])
...
def display_books(df: pd.DataFrame, key_prefix: str):
    ...
    for _, row in df.iterrows():
        ...
        with col:
            ...
            if col.button(
                f"{row['title']}", key=f"{key_prefix}_{row['ISBN']}", type="primary"
            ):
